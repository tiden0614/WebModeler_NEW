define(["Kinetic", "Hammer", "WMGroup", "WMUtils", "WMRelation"],
    ...
    var WMComponentIdCount = 0;
    var WMComponentMap = {};
    var WMComponentStorage = [];
    var WMRelationStorage = [];
    var attrHeight = 20;
    var defaultComponentWidth = 180;
    var defaultComponentHeight = 105;
    var defaultHugeComponentWidth = 300;
    var defaultHugeComponentHeight = 300;
    var decoratorFixX = -40;
    var decoratorFixY = 10;
    var triggerDeltaX = 5;
    var triggerDeltaY = 5;
    var scalingFactor = 0.01;
    var areaThreshold = defaultHugeComponentWidth * defaultHugeComponentHeight;
    var scalingAnimationDuration = 200;
    var defaultLineHeadImgSrc = "icons/blank.png";
    var defaultLineTailImgSrc = "icons/blank.png";
    var defaultLineMiddleImgSrc = "icons/component-rel1.png";
    var defaultLineMiddleImgSrcRev = "icons/component-rel1-rev.png";
    var defaultLineName = " ";
    var componentLogoImgSrc = "icons/component-logo.png";
    var debugLogger = WMUtils.getLogger({name: "WMComponent", level: "DEBUG", on: true});
    var eventLogger = WMUtils.getLogger({name: "WMComponent", level: "EVENT", on: true});
    var errorLogger = WMUtils.getLogger({name: "WMComponent", level: "ERROR", on: true});
    var stage = null;
    ...
    var switchLineEndTypeAction = function(type){
        // (function(type){
            var group = this.getParent();
            var lineMiddleStatus = group.WMLineMiddleStatus = 1 - group.WMLineMiddleStatus;
            lineMiddle = group.WMGetComponent("lineMiddle");
            if(lineMiddle != null){
                if(lineMiddle == 0) {
                    lineMiddle.WMLoadImg(defaultLineMiddleImgSrc);
                } else {
                    lineMiddle.WMLoadImg(defaultLineMiddleImgSrcRev);
                }
            }
        // })(type);
    };
    ...
    var maskThis = function(){
        ...
        (function(){
            ...
            inputObj.on("input", function(){
                ...
                //     x: defaultComponentWidth / 2 - w / 2,
                //     y: defaultComponentHeight / 2 - h / 2
                // });
    ...
    /* 生成Component的初始显示组件 */
    var generateNewWMComponentInnerComponents = shapeGeneratorMap["D"] = function(config){
        debugLogger.log("Constructing new WMComponent: " + WMUtils.getObjectStr(config));
        var _id = WMComponentIdCount++;
        config = WMUtils.validateConfig(config, {name: "Component" + _id});
        ...
        // });
        var decorator = WMUtils.getImage({
            x: defaultComponentWidth + decoratorFixX, y: decoratorFixY,
            width: 30, height: 30, src: componentLogoImgSrc
        });
        var rect = new Kinetic.Rect({
            x: 0, y: 0, width: defaultComponentWidth, height: defaultComponentHeight,
        ...
        (function(){
            ...
            group.defaultWidth = defaultComponentWidth;
            group.defaultHeight = defaultComponentHeight;
            nameText.setX(defaultComponentWidth / 2 - nameText.getWidth() / 2);
            nameText.setY(defaultComponentHeight / 2 - nameText.getHeight() / 2);
            WMComponentMap["" + _id] = group;
            group.dimensionStatus = 0;
            group.includedComponents = {};
            group.parentComponent = null;
            group.lastMovePoint = {x: 0, y: 0};
            group.dashRelation = false;
            group.dontMove = false;
            group.defaultLineName = defaultLineName;
            group.WMSwitchLineEndTypeAction = switchLineEndTypeAction;
            group.WMLineMiddleStatus = 0;
        })();
    ...
    var newInstance = function(config){
        ...
        if(groupGen == null){
            groupGen = generateNewWMComponentInnerComponents;
        }
        ...
        hammer.on("touchmove", function(e){
            ...
            if(e.touches.length == 1 && !this.editable
                ...
                if(this.dimensionStatus == 1){
                    /* 移动为包含的Component */
                    for(var _packageId in this.includedComponents){
                        var _package = this.includedComponents[_packageId];
                        if(_package != null){
                            _package.move(moveP);
                        }
                    }
        ...
        hammer.on("touchend", function(){
            ...
            if(t && t.dimensionStatus == 1){
                ...
                t.includedComponents["" + this.id] = this;
            } else {
                if(this.parentComponent != null){
                    this.parentComponent["includedComponents"]["" + this.id] = null;
                    ...
                    setTimeout(function(){
                        if(self.holdStart && !self.editable){
                            ...
                            var _newClass = newInstance({
                                x: self.holdPoint["x"] - defaultComponentWidth / 2,
                                y: self.holdPoint["y"] - defaultComponentHeight / 2,
                                editable: false, gestureCreated: true
                            });
                            WMRelation.connect({
                                start: _newClass, end: self,
                                lineMiddleImgSrc: defaultLineMiddleImgSrc,
                                WMSwitchLineEndTypeAction: switchLineEndTypeAction
                            });
        ...
        hammer.on("pinchout", function(e){
            ...
            if(this.dimensionStatus == 0) {
                ...
                if(startScaling){
                    ...
                    var self = this;
                    var rect = this.WMGetComponent("rect");
                    var decorator = this.WMGetComponent("decorator");
                    var wDiff = defaultHugeComponentWidth - defaultComponentWidth;
                    var hDiff = defaultHugeComponentHeight - defaultComponentHeight;
                    var w = rect.getWidth(), h = rect.getHeight();
                    var hugeAnim = new Kinetic.Animation(function(frame){
                        ...
                        if(time < scalingAnimationDuration){
                            ...
                            rect.setWidth(w + _w);
                            rect.setHeight(h + _h);
                            decorator.setX(w + _w + decoratorFixX);
                        } else {
                            rect.setWidth(defaultHugeComponentWidth);
                            rect.setHeight(defaultHugeComponentHeight);
                            decorator.setX(defaultHugeComponentWidth + decoratorFixX);
                            self.move({x: 0, y: 0});
                            this.stop();
                        }
                    }, this.getLayer());
                ...
                rect.setWidth(w);
                rect.setHeight(h);
                this.WMGetComponent("decorator").setX(w + decoratorFixX);
                this.move({x: 0, y: 0});
                this.getLayer().batchDraw();
            }
        });
        ...
        hammer.on("pinchin", function(e){
            ...
            if(this.dimensionStatus == 1){
                ...
                var w = rect.getWidth() * scaleFixed;
                var h = rect.getHeight() * scaleFixed;
                if(w >= defaultHugeComponentWidth || h >= defaultHugeComponentHeight){
                    if(w >= defaultHugeComponentWidth){
                        rect.setWidth(w);
                    }
                    if(h >= defaultHugeComponentHeight){
                        rect.setHeight(h);
                    }
                    this.move({x: 0, y: 0});
                    decorator.setX(w + decoratorFixX);
                    this.getLayer().batchDraw();
                } else {
                    this.dontMove = true;
                    this.dimensionStatus = 0;
                    var self = this;
                    var wDiff = w - defaultComponentWidth;
                    var hDiff = h - defaultComponentHeight;
                    var hugeAnim = new Kinetic.Animation(function(frame){
                        var time = frame.time;
                        if(time < scalingAnimationDuration){
                            var _w = wDiff * (time / scalingAnimationDuration);
                            ...
                            rect.setWidth(defaultComponentWidth);
                            rect.setHeight(defaultComponentHeight);
                            decorator.setX(defaultComponentWidth + decoratorFixX);
                            self.move({x: 0, y: 0});
                            this.stop();
                        }
                    }, this.getLayer());
        ...
        if(t && t.dimensionStatus == 1){
            group.parentComponent = t;
            t.includedComponents["" + group.id] = group;
        } else {
            if(group.parentComponent != null){
                group.parentComponent["includedComponents"]["" + group.id] = null;
                group.parentComponent = null;
            }
        }
        WMComponentStorage.push(group);
        return group;
    };

    var getInstanceFromPoint = function(stagePoint, src){
        hitClass = null;
        for(var i = 0; i < WMComponentStorage.length; i++){
            var wmclass = WMComponentStorage[i];
    ...
    return {
        ...
        getDefaultLineMiddleImgSrc: function(){
            return defaultLineMiddleImgSrc;
        },
